  ...
  return (
    <ThemeScopeProvider value={scopeEl}>
      <div ref={setScopeEl} className="relative h-dvh overflow-hidden" style={style}>
        <header className={cn('bg-background/82 fixed inset-x-0 top-0 z-30 h-16 border-b border-border/75 shadow-[0_1px_0_rgba(255,255,255,0.35)] backdrop-blur-2xl', headerClassName)}>
...
export function AppFooter({ children, className }: { children?: ReactNode; className?: string }) {
  return (
    <footer className={cn('bg-background/82 text-muted-foreground fixed inset-x-0 bottom-0 z-30 flex h-11 items-center justify-center border-t border-border/75 px-4 text-center text-xs backdrop-blur-2xl', className)}>
